...
import { projectAssembled, blocksOrderSelector } from "visual/redux/selectors";
...
export default historyReducerEnhancer(
  ...
  ),
  {
    ...
    onBeforeUpdate: (state, action, history) => {
      if (action.type === UPDATE_GB_RULES || action.type === REMOVE_BLOCK) {
        const { id } = action.payload;
        const blocksOrder = blocksOrderSelector(state);

        if (blocksOrder.includes(id)) {
