...
class MenuItem extends EditorComponent {
  ...
  getMegaMenuWidth = (width, suffix) => {
    ...
    if (leftSpaces < calcWidth && rightSpaces < calcWidth) {
  ...
  renderMegaMenuForEdit(v, vs, vd) {
    ...
    if (mode === "vertical") {
      const megaMenuWidth = styleElementMegaMenuWidth({ v, device });
      const megaMenuWidthSuffix = styleElementMegaMenuWidthSuffix({
        v,
        device
      });
      props = {
      ...
      maxWidth = this.getMegaMenuWidth(megaMenuWidth, megaMenuWidthSuffix);
    } else {
      const { width = 0 } = parentNode?.getBoundingClientRect();
      props = {
        placement: "bottom",
