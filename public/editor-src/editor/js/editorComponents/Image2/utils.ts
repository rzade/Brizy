...
export const calcImageSizes = (
  ...
  if (v.imageWidth / wrapperSizes.width < v.imageHeight / wrapperSizes.height) {
    widthCoefficient = wrapperSizes.width / v.imageWidth;
  } else {
    heightCoefficient = wrapperSizes.height / v.imageHeight;
  }
  ...
  const containerWidth = wrapperSizes.width * (v.zoom / 100);
  const containerHeight = wrapperSizes.height * (v.zoom / 100);
