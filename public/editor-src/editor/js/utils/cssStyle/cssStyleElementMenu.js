...
export function cssStyleElementMenuMode({ v, device, state }) {
  ...
  if (mode === "horizontal") {
    return "display: flex; flex-wrap: wrap; justify-content: inherit;";
  }
...
export function cssStyleElementMenuLinkBorder({ v, device, state }) {
  const mode = styleElementMenuMode({ v, device, state });
...
export function cssStyleElementMenuActiveLinkBorder({ v, device, state }) {
  const mode = styleElementMenuMode({ v, device, state });
...
export function cssStyleElementMMenuBtnNext({ v, device }) {
  ...
  const height = Math.round(mMenuLineHeight * mMenuFontSize * 10) / 10;

  return `height: calc(${height}px + 19px);`;
}
